...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mei="http://www.music-encoding.org/ns/mei" xmlns:svg="http://www.w3.org/2000/svg" xmlns:math="http://www.w3.org/2005/xpath-functions/math" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:local="local" exclude-result-prefixes="xs math xd mei svg" version="3.0">
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p>
                This stylesheet gets an @id of a given MEI element and returns information about it.
            </xd:p>
            <xd:p>
    ...
    <xsl:function name="local:getLocal" as="xs:string">
        ...
        <xsl:value-of select="local:getLocal($lang,$key,'','','','')"/>
    </xsl:function>
    ...
    <xsl:function name="local:getLocal" as="xs:string">
        ...
        <xsl:value-of select="local:getLocal($lang,$key,$param1,'','','')"/>
    </xsl:function>
    ...
    <xsl:function name="local:getLocal" as="xs:string">
        ...
        <xsl:value-of select="local:getLocal($lang,$key,$param1,$param2,'','')"/>
    </xsl:function>
    ...
    <xsl:function name="local:getLocal" as="xs:string">
        ...
        <xsl:value-of select="local:getLocal($lang,$key,$param1,$param2,$param3,'')"/>
    </xsl:function>
        ...
        <xsl:variable name="strings" as="xs:string*">
            <xsl:choose>
                ...
                <xsl:when test="count($modifications) = 0">
                    <xsl:value-of select="local:getLocal($lang,'element.neverModified')"/>
                </xsl:when>
                
                <!-- element modified -->
                <xsl:otherwise>
                    <xsl:if test="not($modifications[local-name() = 'add'])">
                        <xsl:value-of select="local:getLocal($lang,'alreadyInBaseLayer',$doc//mei:genDesc[@ordered = 'true' and @plist]/mei:state[1]/@label)"/>
                    </xsl:if>
                    ...
                    <xsl:for-each select="$involved.states">
                        ...
                        <xsl:choose>
                            <xsl:when test="count($current.modifications) = 1">
                                <xsl:value-of select="local:describeModification($current.modifications,$lang)"/>
                            </xsl:when>
                            <xsl:when test="exists($current.modifications[local-name() = 'del']) and exists($current.modifications[local-name() = 'restore'])">
                                ...
                                <xsl:value-of select="local:describeModification($combined.mod,$lang)"/>
                            </xsl:when>
                            <xsl:when test="exists($current.modifications[local-name() = 'add']) and exists($current.modifications[local-name() = 'restore'])">
                                <xsl:value-of select="local:describeModification($current.modifications[local-name() = 'add'],$lang)"/>
                            </xsl:when>
    ...
    <xsl:function name="local:describeModification" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="local-name($modification) = 'add'">
                <xsl:value-of select="local:getLocal($lang,'addedIn',$state/@label)"/>
            </xsl:when>
            <xsl:when test="local-name($modification) = 'del'">
                <xsl:value-of select="local:getLocal($lang,'deletedIn',$state/@label)"/>
            </xsl:when>
            <xsl:when test="local-name($modification) = 'restore'">
                <xsl:value-of select="local:getLocal($lang,'restoredIn',$state/@label)"/>
            </xsl:when>
            <xsl:when test="local-name($modification) = 'restoreDel'">
                <xsl:value-of select="local:getLocal($lang,'restoredAndDeleted',$state/@label)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="local:getLocal($lang,'modifiedIn',$state/@label,local-name($modification))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="local:getMeasure" as="xs:string">
        ...
        <xsl:choose>
            <xsl:when test="$elem/ancestor::mei:measure">
                <xsl:value-of select="concat(local:getLocal($lang,'measure',$elem/ancestor::mei:measure/@label),$staff.ref)"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="''"/>
            </xsl:otherwise>
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="every $id in $affected.measures satisfies exists($doc//mei:measure[@xml:id = $id and preceding::mei:measure/@xml:id = $measure/@xml:id])">
                        <xsl:value-of select="local:getLocal($lang,'precedes.störstelle')"/>
                    </xsl:when>
                    ...
                    <xsl:when test="every $id in $affected.measures satisfies exists($doc//mei:measure[@xml:id = $id and following::mei:measure/@xml:id = $measure/@xml:id])">
                        <xsl:value-of select="local:getLocal($lang,'follows.störstelle')"/>
                    </xsl:when>
    ...
    <xsl:function name="local:processNote" as="xs:string">
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($note,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;note&#34;,',             '&#34;id&#34;:&#34;',$note/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($note,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($note,$lang),'&#34;,',             '&#34;bravura&#34;:&#34;',$dur,$dotted,'&#34;,',             '&#34;desc&#34;:&#34;',$pitch,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processChord" as="xs:string">
        ...
        <xsl:variable name="dotted" as="xs:string*">
            <xsl:choose>
                ...
                <xsl:when test="$chord/@dots = '1'">
                    <xsl:value-of select="local:getLocal($lang,'dots.1')"/>
                </xsl:when>
                <xsl:when test="$chord/@dots = '2'">
                    <xsl:value-of select="local:getLocal($lang,'dots.2')"/>
                </xsl:when>
                <xsl:when test="$chord/@dots = '3'">
                    <xsl:value-of select="local:getLocal($lang,'dots.3')"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="dur" as="xs:string">
            <xsl:choose>
                <xsl:when test="$chord/@dur = 1">
                    <xsl:value-of select="local:getLocal($lang,'dur.1')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 2">
                    <xsl:value-of select="local:getLocal($lang,'dur.2')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 4">
                    <xsl:value-of select="local:getLocal($lang,'dur.4')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 8">
                    <xsl:value-of select="local:getLocal($lang,'dur.8')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 16">
                    <xsl:value-of select="local:getLocal($lang,'dur.16')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 32">
                    <xsl:value-of select="local:getLocal($lang,'dur.32')"/>
                </xsl:when>
                <xsl:when test="$chord/@dur = 64">
                    <xsl:value-of select="local:getLocal($lang,'dur.64')"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($chord,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;chord&#34;,',             '&#34;id&#34;:&#34;',$chord/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',                         '&#34;measure&#34;:&#34;',local:getMeasure($chord,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($chord,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',$dotted,' ',local:getLocal($lang,'chord'),' ',$dur,' ',string-join($pitches,', '),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processBeam" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($beam,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;beam&#34;,',             '&#34;id&#34;:&#34;',$beam/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',                         '&#34;measure&#34;:&#34;',local:getMeasure($beam,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($beam,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'beam'),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processDel" as="xs:string">
        ...
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;del&#34;,',             '&#34;id&#34;:&#34;',$del/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',                         '&#34;measure&#34;:&#34;',local:getMeasure($del,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($del,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'deletionIn'),' ',$stateLabel,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processAccid" as="xs:string">
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($accid,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;accid&#34;,',             '&#34;id&#34;:&#34;',$accid/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;',$bravura,'&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($accid,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($accid,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'accidental'),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processRest" as="xs:string">
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($rest,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;rest&#34;,',             '&#34;id&#34;:&#34;',$rest/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;',$dur,$dotted,'&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($rest,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($rest,$lang),'&#34;,',             '&#34;desc&#34;:&#34;&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processClef" as="xs:string">
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($clef,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;clef&#34;,',             '&#34;id&#34;:&#34;',$clef/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;',$bravura,'&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($clef,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($clef,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',$clef/@shape,local:getLocal($lang,'clef.combined'),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processDir" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($dir,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;dir&#34;,',             '&#34;id&#34;:&#34;',$dir/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($dir,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($dir,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'directive'),' ',string-join($dir//text(),' '),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processOctave" as="xs:string">
        ...
        <xsl:variable name="stateDesc" select="local:getStateDesc($octave,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;octave&#34;,',             '&#34;id&#34;:&#34;',$octave/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',              '&#34;bravura&#34;:&#34;',$bravura,'&#34;,',                '&#34;measure&#34;:&#34;',local:getMeasure($octave,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($octave,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'octave.dir'),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processSlur" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($slur,$lang)" as="xs:string"/>
        <xsl:variable name="endLabel" as="xs:string">
            <xsl:choose>
                <xsl:when test="starts-with($slur/@tstamp2,'0m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstamp',substring-after($slur/@tstamp2,'0m+')))"/>
                </xsl:when>
                <xsl:when test="starts-with($slur/@tstamp2,'1m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstampOfNextMeasure',substring-after($slur/@tstamp2,'1m+')))"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat(' ',local:getLocal($lang,'endingAtTstampOfNextMeasure',local:getOrdinal(substring-before($slur/@tstamp2,'m+'),$lang),substring-after($slur/@tstamp2,'1m+')))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;slur&#34;,',             '&#34;id&#34;:&#34;',$slur/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($slur,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($slur,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'slur'),' ',local:getLocal($lang,'startingAtTstamp',$slur/@tstamp),' ',$endLabel,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processTie" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($tie,$lang)" as="xs:string"/>
        <xsl:variable name="endLabel" as="xs:string">
            <xsl:choose>
                <xsl:when test="starts-with($tie/@tstamp2,'0m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstamp',substring-after($tie/@tstamp2,'0m+')))"/>
                </xsl:when>
                <xsl:when test="starts-with($tie/@tstamp2,'1m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstampOfNextMeasure',substring-after($tie/@tstamp2,'1m+')))"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat(' ',local:getLocal($lang,'endingAtTstampOfNextMeasure',local:getOrdinal(substring-before($tie/@tstamp2,'m+'),$lang),substring-after($tie/@tstamp2,'1m+')))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;tie&#34;,',             '&#34;id&#34;:&#34;',$tie/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($tie,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($tie,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'tie'),' ',local:getLocal($lang,'startingAtTstamp',$tie/@tstamp),' ',$endLabel,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processDynam" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($dynam,$lang)" as="xs:string"/>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;dynam&#34;,',             '&#34;id&#34;:&#34;',$dynam/@xml:id,'&#34;,',                  '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',                  '&#34;bravura&#34;:&#34;&#34;,',                     '&#34;measure&#34;:&#34;',local:getMeasure($dynam,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($dynam,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',local:getLocal($lang,'dynamic.dir'),' ',string-join($dynam//text(),' '),'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processHairpin" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($hairpin,$lang)" as="xs:string"/>
        <xsl:variable name="desc" as="xs:string">
            <xsl:choose>
                <xsl:when test="$hairpin/@form = 'dim'">
                    <xsl:value-of select="local:getLocal($lang,'hairpin.dim')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="local:getLocal($lang,'hairpin.cres')"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="endLabel" as="xs:string">
            <xsl:choose>
                <xsl:when test="starts-with($hairpin/@tstamp2,'0m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstamp',substring-after($hairpin/@tstamp2,'0m+')))"/>
                </xsl:when>
                <xsl:when test="starts-with($hairpin/@tstamp2,'1m')">
                    <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstampOfNextMeasure',substring-after($hairpin/@tstamp2,'1m+')))"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat(' ',local:getLocal($lang,'endingAtTstampOfNextMeasure',local:getOrdinal(substring-before($hairpin/@tstamp2,'m+'),$lang),substring-after($hairpin/@tstamp2,'1m+')))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;hairpin&#34;,',             '&#34;id&#34;:&#34;',$hairpin/@xml:id,'&#34;,',                  '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',                  '&#34;bravura&#34;:&#34;&#34;,',                     '&#34;measure&#34;:&#34;',local:getMeasure($hairpin,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($hairpin,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',$desc,' ',local:getLocal($lang,'startingAtTstamp',$hairpin/@tstamp),' ',$endLabel,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processArtic" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($artic,$lang)" as="xs:string"/>
        <xsl:variable name="desc" as="xs:string">
            <xsl:choose>
                <xsl:when test="$artic/@artic = 'dot'">
                    <xsl:value-of select="local:getLocal($lang,'artic.dot')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="local:getLocal($lang,'artic',string($artic/@artic))"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;artic&#34;,',             '&#34;id&#34;:&#34;',$artic/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($artic,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($artic,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',$desc,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processMetaMark" as="xs:string">
        ...
        <xsl:variable name="function">
            <xsl:choose>
                <xsl:when test="$metaMark/@function = 'navigation'">
                    <xsl:choose>
                        <xsl:when test="$metaMark/@target">
                            <xsl:value-of select="local:getLocal($lang,'link.origin')"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="local:getLocal($lang,'link.target')"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
                <xsl:when test="$metaMark/@function = 'confirmation'">
                    ...
                    <xsl:variable name="typeString" as="xs:string">
                        <xsl:choose>
                            <xsl:when test="local-name($target) = 'del'">
                                <xsl:value-of select="local:getLocal($lang,'confirmation.deletion')"/>
                            </xsl:when>
                            <xsl:when test="local-name($target) = 'restore'">
                                <xsl:value-of select="local:getLocal($lang,'confirmation.restore')"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="''"/>
                            </xsl:otherwise>
                    ...
                    <xsl:value-of select="concat(local:getLocal($lang,'confirmation'),$typeString)"/>
                </xsl:when>
                <xsl:when test="$metaMark/@function = 'clarification'">
                    <xsl:choose>
                        <xsl:when test="$metaMark//text()">
                            <xsl:value-of select="local:getLocal($lang,'clarification',string-join($metaMark//text(),' '))"/>
                        </xsl:when>
                        <xsl:when test="$metaMark/@target">
                            <xsl:variable name="target" select="$doc/id(substring-after($metaMark/@target,'#'))" as="node()"/>
                            <xsl:value-of select="local:getLocal($lang,'clarification',local:getLocal($lang,local-name($target)))"/>
                        </xsl:when>
                    </xsl:choose>
                </xsl:when>
                <xsl:when test="$metaMark/@function = 'restoration'">
                    <xsl:value-of select="local:getLocal($lang,'restoration',string-join($metaMark//text(),' '))"/>
                </xsl:when>
                <xsl:when test="$metaMark/@function = 'deletion'">
                    <xsl:value-of select="local:getLocal($lang,'deletion.text',string-join($metaMark//text(),' '))"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="stateDesc" select="local:getStateDesc($metaMark,$lang)" as="xs:string"/>
        ...
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;metaMark&#34;,',              '&#34;id&#34;:&#34;',$metaMark/@xml:id,'&#34;,',              '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',                 '&#34;measure&#34;:&#34;',local:getMeasure($metaMark,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($metaMark,$lang),'&#34;,',             '&#34;target&#34;:&#34;',$target,'&#34;,',                '&#34;desc&#34;:&#34;',$function,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processCpMark" as="xs:string">
        ...
        <xsl:param name="lang" as="xs:string" required="yes"/>
        <xsl:variable name="stateDesc" select="local:getStateDesc($cpMark,$lang)" as="xs:string"/>
        <xsl:variable name="desc" as="xs:string">
            <xsl:choose>
                <xsl:when test="$cpMark/@ref.staff">
                    ...
                    <xsl:variable name="startLabel" select="local:getLocal($lang,'startingAtTstamp',$cpMark/@tstamp)" as="xs:string"/>
                    <xsl:variable name="endLabel" as="xs:string">
                        <xsl:choose>
                            <xsl:when test="starts-with($cpMark/@tstamp2,'0m')">
                                <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstamp',substring-after($cpMark/@tstamp2,'0m+')))"/>
                            </xsl:when>
                            <xsl:when test="starts-with($cpMark/@tstamp2,'1m')">
                                <xsl:value-of select="concat(' ', local:getLocal($lang,'endingAtTstampOfNextMeasure',substring-after($cpMark/@tstamp2,'1m+')))"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="concat(' ',local:getLocal($lang,'endingAtTstampOfNextMeasure',local:getOrdinal(substring-before($cpMark/@tstamp2,'m+'),$lang),substring-after($cpMark/@tstamp2,'1m+')))"/>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>
                    <xsl:value-of select="local:getLocal($lang,'cpMark.collaParte',$other.staff,$startLabel,$endLabel,if($cpMark/@dis) then(local:getLocal($lang,'in8va')) else(''))"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="local:getLocal($lang,'cpMark')"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;cpMark&#34;,',             '&#34;id&#34;:&#34;',$cpMark/@xml:id,'&#34;,',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;',local:getMeasure($cpMark,$lang),'&#34;,',             '&#34;position&#34;:&#34;',local:qualifyPosition($cpMark,$lang),'&#34;,',             '&#34;desc&#34;:&#34;',$desc,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:function name="local:processBarline" as="xs:string">
        ...
        <xsl:variable name="desc" as="xs:string">
            <xsl:choose>
                <xsl:when test="count($all.measures) = 1">
                    <xsl:value-of select="local:getLocal($lang,'barline.atMeasure',$first.measure/@label)"/>
                </xsl:when>
                <xsl:when test="count($all.measures) = 2">
                    ...
                    <xsl:value-of select="local:getLocal($lang,'barline.betweenMeasures',$measures.sorted[1]/@label,$measures.sorted[2]/@label)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="''"/>
                </xsl:otherwise>
        ...
        <xsl:value-of select="concat('{&#34;lang&#34;:&#34;',$lang,'&#34;,&#34;type&#34;:&#34;barline&#34;,',             '&#34;id&#34;:&#34;',$first.measure/@xml:id,'&#34;,',             '&#34;svgIDs&#34;:[&#34;',string-join($svgIDs,'&#34;,&#34;'),'&#34;],',             '&#34;stateDesc&#34;:&#34;',$stateDesc,'&#34;,',             '&#34;bravura&#34;:&#34;&#34;,',             '&#34;measure&#34;:&#34;','&#34;,',             '&#34;position&#34;:&#34;','&#34;,',             '&#34;desc&#34;:&#34;',$desc,'&#34;}')"/>
    </xsl:function>
    ...
    <xsl:variable name="languages" select="('de','en')" as="xs:string*"/>
    
    <!-- start processing -->
    <xsl:template match="/">
        <xsl:variable name="elems" as="node()*">
            <xsl:choose>
                <xsl:when test="//svg:path[@id = $element.id]">
                    <!-- gets all MEI elements that reference this particular svg path -->
                    <xsl:sequence select="//mei:*[concat('#',$element.id) = tokenize(@facs,' ')]"/>
                </xsl:when>
                <xsl:when test="//mei:*[@xml:id = $element.id]">
                    <!-- get the MEI element itself -->
                    <xsl:sequence select="//mei:*[@xml:id = $element.id]"/>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="strings" as="xs:string*">
            <xsl:for-each select="$languages">
                <xsl:variable name="current.lang" select="." as="xs:string"/>
                <xsl:for-each select="$elems">
                    <xsl:choose>
                        <xsl:when test="local-name(.) = 'metaMark'">
                            <xsl:value-of select="local:processMetaMark(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'rend' and ancestor::mei:metaMark">
                            <xsl:value-of select="local:processMetaMark(ancestor::mei:metaMark[1],$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'del'">
                            <xsl:value-of select="local:processDel(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'beam'">
                            <xsl:value-of select="local:processBeam(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'note'">
                            <xsl:value-of select="local:processNote(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'accid'">
                            <xsl:value-of select="local:processAccid(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'dir'">
                            <xsl:value-of select="local:processDir(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'octave'">
                            <xsl:value-of select="local:processOctave(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'chord'">
                            <xsl:value-of select="local:processChord(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'rest'">
                            <xsl:value-of select="local:processRest(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'clef'">
                            <xsl:value-of select="local:processClef(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'slur'">
                            <xsl:value-of select="local:processSlur(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'tie'">
                            <xsl:value-of select="local:processTie(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'beamSpan'">
                            <xsl:value-of select="local:processBeam(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'dynam'">
                            <xsl:value-of select="local:processDynam(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'hairpin'">
                            <xsl:value-of select="local:processHairpin(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'artic'">
                            <xsl:value-of select="local:processArtic(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'cpMark'">
                            <xsl:value-of select="local:processCpMark(.,$current.lang)"/>
                        </xsl:when>
                        <xsl:when test="local-name(.) = 'measure'">
                            <xsl:value-of select="local:processBarline(.,$elems,$element.id,$current.lang)"/>
                        </xsl:when>
                        <xsl:otherwise/>
                    </xsl:choose>
                </xsl:for-each>

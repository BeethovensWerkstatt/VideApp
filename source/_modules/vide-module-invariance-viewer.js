...
const VideInvarianceViewer = class VideInvarianceViewer extends EoModule {

    /*Constructor method*/
    constructor() {
        super();
        this._supportedPerspective = EO_Protocol.Perspective.PERSPECTIVE_INVARIANCE;
        ...
        this._key = 'videInvarianceViewer';
        this._serverConfig = {host: 'http://localhost', port: ':33098', basepath:'/'};
        
        this._stateStore = new Map();
        this._currentRendering = new Map();
        this._invarianceRelations = new Map();
    ...
    handleRequest(request) {
        ...
        let containerID = (reqContainer.endsWith('_VIEWTYPE_INVARIANCEVIEW')) ? reqContainer : reqContainer + '_VIEWTYPE_INVARIANCEVIEW';
        ...
        return Promise.resolve(
            this._setupMenu(containerID, activeStateIDs, mainStateID)
                ...
                //prepare url
                    ...
                    let responseType = 'text';
                    let url = this._getBaseURI() + 'edition/' + editionID + '/state/' + mainState + '/otherStates/' + otherStates + '/meiSnippet.xml';
                ...
                .then((svg) => {
                    return this._getInvarianceRelations(editionID, containerID)
                        .then((invarianceData) => {
                            console.log('-------------------------------------------41');
                            console.log(invarianceData);
                        
                            let colors = ['#4e9bba', '#c380dd', '#c63e00', '#6fdb3d', '#b75122', '#5a6ece', '#c10937', '#cb25d1', '#ce6c5a', '#2140dd', '#2b88af', '#4e50c4', '#859900', '#33b0ea', '#268bd2', '#bc6827', '#2aa198', '#cb4b16', '#6c71c4', '#d33682', '#dc322f', '#b58900'];//randomColor({count: 20,hue: 'monochrome'});//['#859900','#d33682','#2aa198','#cb4b16','#268bd2','#6c71c4','#dc322f','#b58900'];
                            let suppliedColor = '#999999';
                        
                        
                        
                            for(let i = 0; i < invarianceData.baseStates.length; i++) {
                                let baseState = invarianceData.baseStates[i];
                                let stateIndex = invarianceData.states.indexOf(baseState.stateID);
                            
                                try {
                                    let elem = document.querySelector('#' + containerID + ' svg #' + baseState.id);
                                    if(elem !== null) {
                                        elem.setAttribute('fill', colors[stateIndex]);
                                        elem.setAttribute('stroke', colors[stateIndex]);
                                    }
                                } catch(err) {
                                    console.log('[ERROR] Unable to highlight baseState ' + baseState.id + ' (' + i + ') in ' + colors[stateIndex] + ' | ' + err);
                                }
                            }
                        
                            for(let i = 0; i < invarianceData.relations.length; i++) {
                                let relation = invarianceData.relations[i];
                                let stateIndex = invarianceData.states.indexOf(relation.originState);
                            
                                try {
                                    let elem = document.querySelector('#' + containerID + ' svg #' + relation.targetID);
                                    if(elem !== null) {
                                        elem.setAttribute('fill', colors[stateIndex]);
                                        elem.setAttribute('stroke', colors[stateIndex]);    
                                    }
                                } catch(err) {
                                    console.log('[ERROR] Unable to highlight relation ' + relation.targetID + ' in ' + colors[stateIndex] + ' | ' + err);
                                }
                            }
                            ...
                            try {
                                let clef = document.querySelector('#' + containerID + ' svg g.measure:first-of-type .layer g.clef:first-of-type');
                                if(clef !== null) {
                                    clef.setAttribute('fill', suppliedColor);
                                    clef.setAttribute('stroke', suppliedColor);
                                }
                                let meterSig = document.querySelector('#' + containerID + ' svg g.measure:first-of-type .layer g.clef:first-of-type + g.meterSig');
                                if(meterSig !== null) {
                                    meterSig.setAttribute('fill', suppliedColor);
                                    meterSig.setAttribute('stroke', suppliedColor);
                                }
                                let keySig = document.querySelector('#' + containerID + ' svg g.measure:first-of-type .layer g.clef:first-of-type + g.meterSig + g.keySig');
                                if(keySig !== null) {
                                    keySig.setAttribute('fill', suppliedColor);
                                    keySig.setAttribute('stroke', suppliedColor);
                                }
                            } catch(err) {
                                console.log('[ERROR] Unable to display scoreDef as supplied: ' + err);
                            }
    ...
    _getInvarianceRelations(editionID, containerID) {
        if(this._invarianceRelations.has(editionID)) {
            return Promise.resolve(this._invarianceRelations.get(editionID));
        } else {
            let relationsType = 'json';
            let relationsUrl = this._getBaseURI() + 'edition/' + this._eohub.getEdition() + '/invarianceRelations.json';
            
            let relationsRequest = new DataRequest(relationsType, relationsUrl);
            return this._eohub.requestData(relationsRequest, containerID)
                .then((json) => {
                    this._invarianceRelations.set(editionID, json);
...
export default VideInvarianceViewer;
